...
const Search = () => {
    ...
    useEffect(() => {
        const fetchGames = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/search?phrase=${encodeURIComponent(searchTerm)}`);
        ...
        const filterGamesByGenre = async () => {
            ...
            const gamesWithGenreInfo = await Promise.all(
                allBoardGames.map(async (game) => {
                    try {
                        const detailsResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/gameDetails?id=${game.id}`);
                        const gameDetails = await detailsResponse.json();
                        const hasSelectedGenre = gameDetails.genres?.includes(selectedGenre);
                        return { ...game, hasSelectedGenre };
                    } catch {
                        return { ...game, hasSelectedGenre: false };
                    }
                })
            );
